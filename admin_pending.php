...
$query = "SELECT * FROM form WHERE status = '$filter' OR (status IS NULL AND '$filter' = 'pending') ORDER BY created_at DESC";
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .table thead th { background: var(--light-bg); border: none; font-weight: 600; color: var(--text-primary); padding: 1rem; font-size: 0.875rem; text-transform: uppercase; }
...
<body>
  <div class="sidebar">
    ...
    <nav class="sidebar-nav">
      ...
      <a href="admin_pending.php" class="active"><i class="fa-solid fa-clock"></i><span>Pending Requests</span></a>
  ...
  <div class="main-content">
    <div class="top-navbar">
      <div class="d-flex justify-content-between align-items-center">
        <h1 class="mb-0 h4"><i class="fa-solid fa-clock me-2 text-warning"></i> Pending Requests</h1>
    ...
    <div class="table-container">
      ...
      <div class="table-responsive">
        <table class="table align-middle">
          <thead><tr><th>#</th><th>Application ID</th><th>Name</th><th>Email</th><th>Department</th><th>Date</th><th>Actions</th></tr></thead>
          <tbody>
            <?php 
            $i = 1;
            ...
            <tr>
              ...
              <td>
                <button class="btn btn-sm btn-success" onclick="updateStatus(<?php echo $row['id']; ?>, 'approved')"><i class="fa-solid fa-check"></i></button>
                <button class="btn btn-sm btn-danger" onclick="updateStatus(<?php echo $row['id']; ?>, 'rejected')"><i class="fa-solid fa-times"></i></button>
                <a href="view_application.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-info"><i class="fa-solid fa-eye"></i></a>
              </td>
            </tr>
            <?php }} else { echo '<tr><td colspan="7" class="text-center">No pending applications</td></tr>'; } ?>
          </tbody>
        </table>
      </div>
    </div>
  ...
  <script>
    ...
    function updateStatus(formId, status) {
      if(confirm(`Are you sure you want to ${status} this application?`)) {
        ...
        }).then(r => r.json()).then(d => { if(d.success) { alert(`Application ${status}!`); location.reload(); } else { alert('Error: ' + d.message); } });
      }
    }
  </script>
</body>
